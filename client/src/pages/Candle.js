...
export default function Candle() {
  ...
  const handleLightButton = () => {
    ...
    if (form.email !== form.confirmEmail) {
      ...
      setTimeout(() => setEmailMatchError(''), 3000);
      return;
    }
    const updatedPray = `${selectedChurch}, ${form.pray}`;
    navigate('/checkout/candle', { state: { form: { ...form, pray: updatedPray } } });
  };
  ...
  return (
    <Box sx={{ minHeight: '80vh', backgroundColor: '#F7F2E8' }}>
      {/* Header */}
      <Box sx={{
        position: 'relative', height: { xs: 260, md: 340 },
        ...
        '&::after': {
          content: '""', position: 'absolute', inset: 0,
          background: 'linear-gradient(to bottom, rgba(0,0,0,0.15) 0%, transparent 40%, #F7F2E8 100%)',
        },
      }}>
        <Box sx={{ position: 'relative', zIndex: 1, textAlign: 'center', px: 3 }}>
          <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 2, mb: 2 }}>
            ...
            <LocalFireDepartmentIcon sx={{ color: crimson, fontSize: '1.6rem', filter: `drop-shadow(0 0 12px ${alpha(crimson, 0.9)})` }} />
      ...
      <Container maxWidth="xl" sx={{ px: { xs: 2, sm: 3 }, pb: { xs: 8, md: 10 } }}>
        <Grid container spacing={5} alignItems="flex-start">
          ...
          <Grid item xs={12} md={5}>
            ...
            >
              ...
              <FormControl component="fieldset" sx={{ mb: 3, width: '100%' }}>
                <FormLabel sx={{ fontFamily: '"Cinzel", serif', fontSize: '0.7rem', letterSpacing: '0.2em', color: `${goldDark} !important`, textTransform: 'uppercase', mb: 1.5 }}>
                ...
                <RadioGroup value={selectedChurch} onChange={(e) => setSelectedChurch(e.target.value)}>
                  {['Annunciation church', 'Greek orthodox church'].map((church) => (
                    <FormControlLabel
                      ...
                      control={<Radio sx={{ color: alpha(gold, 0.35), '&.Mui-checked': { color: gold }, '& .MuiSvgIcon-root': { fontSize: '1rem' } }} />}
                      label={<Typography sx={{ fontFamily: '"Lato", sans-serif', fontSize: '0.9rem', color: '#5D3E2C' }}>{church}</Typography>}
                    />
                  ))}
                </RadioGroup>
              </FormControl>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <TextField fullWidth label={t('candle.firstName')} name="firstname" value={form.firstname} onChange={handleChangeForm} required size="small" />
                  </Grid>
                  <Grid item xs={6}>
                    <TextField fullWidth label={t('candle.lastName')} name="lastname" value={form.lastname} onChange={handleChangeForm} required size="small" />
                  </Grid>
                </Grid>
                <TextField fullWidth label={t('candle.yourEmail')} type="email" name="email" value={form.email} onChange={handleChangeForm} required size="small" />
                <TextField fullWidth label={t('candle.confirmEmail')} type="email" name="confirmEmail" value={form.confirmEmail} onChange={handleChangeForm} required size="small" />
                <TextField fullWidth label={t('candle.yourPrayer')} name="pray" value={form.pray} onChange={handleChangeForm} required multiline rows={4} inputProps={{ maxLength: 500 }} />
              </Box>

              <Collapse in={!!emailMatchError}><Alert severity="error" sx={{ mt: 2 }}>{emailMatchError}</Alert></Collapse>
              <Collapse in={!!inputWarning}><Alert severity="warning" sx={{ mt: 2 }}>{inputWarning}</Alert></Collapse>
              ...
              <Button
                ...
                sx={{
                  ...
                  '&:hover': { boxShadow: `0 8px 30px ${alpha(gold, 0.5)}`, transform: 'translateY(-2px)' },
                }}
              >
                {t('candle.light')}
              </Button>
          ...
          <Grid item xs={12} md={7}>
            <Box sx={{
              ...
              border: `1px solid ${alpha(gold, 0.18)}`, mb: 4, position: 'relative',
              ...
              <Box component="ol" sx={{ pl: 3, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {['candle.step1', 'candle.step2', 'candle.step3'].map((step, i) => (
                  <Box component="li" key={i} sx={{ fontFamily: '"Lato", sans-serif', fontSize: '0.9rem', color: '#5D3E2C', lineHeight: 1.7 }}>
                    {t(step)}
                  </Box>
                ))}
              </Box>
